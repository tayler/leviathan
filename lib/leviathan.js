...
// usage: `node leviathan.js [100] [www.example.com] [80] [/] [GET]`
...
var Leviathan = function() {
	...
	var HTTP = require("http"),
		Q = require("q"),
		options = {
			hostname: args[3] || 'www.example.com',
			port: args[4] || 80,
			path: args[5] || '/',
			method: args[6] || 'GET'
		};
	...
	var oneRequest = function(iteration) {
		...
		var req = HTTP.request(options, function(res) {
			...
			// the value passed into .then() after Q.all() is an array with each promise returned by this function;
	...
	this.runRequests = function(requestCount) {
		...
		.then(function(results) {
			...
			console.log(results);
			console.log('Total time to finish all calls: ' + (callsEndedAt - callsBeganAt));
			results.forEach(function(result) {
				durations.push(result.duration);
				durationTotals += result.duration;
			});
			console.log('Average request duration: ' + durationTotals / requests);
